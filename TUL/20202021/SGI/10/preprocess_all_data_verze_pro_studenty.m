...
for n = 1:pocet_nahravek 
     try
        ...
        %[sig, Fs] = audioread("samples\p0635\c4_p0635_s01.wav");%soubory(n, :));  % nacte nahravku
        ...
        for i = 1:pocet_segmentu
            ...
            energie(i) = sum(segment.*segment,'all');
        end
        ...
        % .....
        fID = fopen(strjoin([soubory(n,:),'txt'],'.'),'w');
        for x = 1:64
            for y = 1:64
                fprintf(fID,'%.4f ',priznaky_pro_rozpoznavani(x,y));
            end
            fprintf(fID,'\n');
        end
        fclose(fID);
        ...
        for i = 1:pocet_segmentu_ve_vyrezu
            spgram(pocet_priznaku_na_segment:-1:1, i) = priznaky_pro_rozpoznavani (i, :);  % spektrum i-teho segmentu 
                  ...
                  % podstati je to otoceni  o 90 stupni do leva
        end
        I = mat2gray(spgram,[0.1 0]);  % zobrazime hodnoty matice pomoci sedotonove stupnice
                                       ...
                                       % Protže vetšina(predpokladam že vsechny hodnoty) je v rozmezi 0,1 až 0
                                       % je to lip viditelne v tomto rozmezi
        ...
        nameChar = char(soubory(n));
        trida = double(str2num(nameChar(k-11))); % identifikator cislice nam bude slouzit pro rozpoznavani
        person = double(str2num(nameChar(k-8:k-5)));  % identifikator osoby
        sada = double(str2num(nameChar(k-2:k-1)));    % identifikator sady
        ...
        close(gcf)   % zavreni obrazku - umite to jinak a lepe?
        %nic lip nez close(HandleNaObrazek) jsem nenasel
    catch E
        soubory(n, :) 
        E.identifier
        pause   
