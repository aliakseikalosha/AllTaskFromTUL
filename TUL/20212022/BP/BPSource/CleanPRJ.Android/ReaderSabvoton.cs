...
namespace DataGrabber.Android
{
    public class ReaderSabvoton : Reader
    {
        ...
        protected override bool CompliteMessage(List<int> message)
        {
            ...
            Debug.WriteLine("SABVOTON"+message.Select(c=>$"{c:X}").Aggregate((a,b)=>a+b));
            return message[0] == SabvotonBluetoothCommand.start && message[^1] == SabvotonBluetoothCommand.end;
        }

        protected override async Task Loop(string name, int sleepTime)
        {
            var prefix = $"{GetType()}";
            IDevice device = null;
            while (canContinue)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            Debug.WriteLine($"{prefix} Connected to sender {sendGuid} and reciver {reciveGuid}");
                        }
