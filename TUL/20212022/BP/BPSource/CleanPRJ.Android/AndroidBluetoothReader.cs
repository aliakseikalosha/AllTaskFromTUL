...
namespace DataGrabber.Android
{
    public class AndroidBluetoothReader : IBluetoothReader
    {
        ...
        public void Init()
        {
            InitReader<Reader>(ref readerBMS, new ReaderBMS(Adapter));
            InitReader<Reader>(ref readerSabvoton, new ReaderSabvoton(Adapter));
        }

        public void InitReader<T>(ref Reader reader, Reader instance) where T: Reader
        {
            reader = instance;
