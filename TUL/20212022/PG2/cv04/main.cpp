...
int main(void) {
    ...
    glm::vec3 lookPos = glm::vec3(0.0f, 0.0f, -5.0f);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        updateInput(window);
        {
            ...
            dt = now - prev;
            prev = now;
            cameraPos += dt * moveDirection(inputState.moveDelta, angle);
            angle += dt * inputState.lookDelta;
            lookPos = cameraPos + glm::vec3(cos(angle.x), 0, sin(angle.x)) * dt;
            glm::mat4 viewMat =
                    glm::lookAt(cameraPos, lookPos, glm::vec3(0.0f, 1.0f, 0.0f));
